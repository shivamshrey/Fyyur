...
# No initial population of the Genre table here
...
class Venue(db.Model):
    ...
    # can references like venue.genres with the above statement
...
class Artist(db.Model):
    ...
    # secondary links this to the associative (m2m) table name
    # can references like artist.genres with the above statement
...
class Show(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    start_time = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)  # required field
    artist_id = db.Column(db.Integer, db.ForeignKey('Artist.id'), nullable=False) # Foreign key is tablename.pk
...
def venues():
  ...
  cities_states = list(cities_states) # convert set into an ordered list
  cities_states.sort(key=itemgetter(1, 0))  # sort by second column (state) first, then by city
  
  now = datetime.now()
  
  for location in cities_states:
    # For this location add if there are any venues there
    venues_list = []
    for venue in venues:
      if venue.city == location[0] and venue.state == location[1]:
        # if there is a venue to add, check how many upcoming shows it has
    ...
    # After all the venues are added to the list for a given location, add it to the data dictionary
    data.append({
      "city": location[0],
      "state": location[1],
      "venues": venues_list
...
def search_venues():
  ...
  # Use filter, not filter_by when doing LIKE search (i = case insensitive)
